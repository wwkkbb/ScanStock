...
public class SystemWebChromeClient extends WebChromeClient {
    ...
    public boolean onShowFileChooser(WebView webView, final ValueCallback<Uri[]> filePathsCallback, final WebChromeClient.FileChooserParams fileChooserParams) {
        ...
        if (isCaptureEnabled && acceptsImagesOnly(acceptTypes)) {
            Intent cameraIntent = createImageCaptureIntent();
            if (cameraIntent != null) {
                try {
                    ...
                    }, cameraIntent, FILECHOOSER_RESULTCODE);
                    return true;
                } catch (ActivityNotFoundException e) {
                    LOG.w(LOG_TAG, "No activity found to handle camera intent.", e);
                    cameraOutputUri = null;
    ...
    private Intent createImageCaptureIntent() {
        ...
        File photoFile;
        try {
            File picturesDir = appContext.getExternalFilesDir(Environment.DIRECTORY_PICTURES);
            if (picturesDir == null) {
                picturesDir = appContext.getCacheDir();
            }
            photoFile = File.createTempFile("cashier_photo_", ".jpg", picturesDir);
        } catch (IOException e) {
            LOG.e(LOG_TAG, "Unable to create temporary image file.", e);
            return null;
        }

        String authority = appContext.getPackageName() + ".fileprovider";
        cameraOutputUri = FileProvider.getUriForFile(appContext, authority, photoFile);
        intent.putExtra(android.provider.MediaStore.EXTRA_OUTPUT, cameraOutputUri);
        intent.addFlags(Intent.FLAG_GRANT_WRITE_URI_PERMISSION | Intent.FLAG_GRANT_READ_URI_PERMISSION);

        for (ResolveInfo resolveInfo : appContext.getPackageManager().queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY)) {
